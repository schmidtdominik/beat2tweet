...
class Spice:
    ...
    def compute_score(self, gts, res):
        ...
        for id in audioIds:
            ...
            input_data.append({
              "image_id" : id,      # This has to be "image_id" for SPICE evaluation to work
              "test" : hypo[0],
              "refs" : ref
            })
        ...
        popen = subprocess.Popen(spice_cmd, cwd=os.path.dirname(os.path.abspath(__file__)),  stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        token_lines,err = popen.communicate()
        print(err)
          # subprocess.check_output(spice_cmd, 
          #   cwd=os.path.dirname(os.path.abspath(__file__)))
        ...
        spice_scores = []
        for item in results:
          audioId_to_scores[item['image_id']] = item['scores']
        ...
        for audio_id in audioIds:
          ...
          score_set = {}
          for category,score_tuple in audioId_to_scores[audio_id].items():
